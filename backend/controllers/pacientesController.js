...
exports.crearPaciente = (req, res) => {
    const { nombre, apellido, documento, telefono, email } = req.body;
    if (!nombre || !apellido) return res.status(400).json({ error: 'Nombre y apellido obligatorios' });

    const sql = 'INSERT INTO pacientes (nombre, apellido, documento, telefono, email) VALUES (?, ?, ?, ?, ?)';
    db.query(sql, [nombre, apellido, documento, telefono, email], (err, result) => {
        if (err) return res.status(500).json(err);
...
exports.actualizarPaciente = (req, res) => {
    const { id } = req.params;
    const { nombre, apellido, documento, telefono, email } = req.body;
    const sql = 'UPDATE pacientes SET nombre=?, apellido=?, documento=?, telefono=?, email=? WHERE id=?';
    db.query(sql, [nombre, apellido, documento, telefono, email, id], (err) => {
        if (err) return res.status(500).json(err);
